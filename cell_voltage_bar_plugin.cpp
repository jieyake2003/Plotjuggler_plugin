...
void CellVoltageBarWidget::drawBars(QPainter& painter, const QRect& rect)
{
    ...
    // Draw Y axis label
    painter.save();
    painter.translate(rect.left() + 8, chartRect.center());
    painter.rotate(-90);
...
QWidget* CellVoltageBarPlugin::embeddedWidget()
{
    if (!m_widget) {
        ...
        m_minSpin->setValue(DEFAULT_Y_MIN);
        m_minSpin->setSingleStep(Y_STEP);
        ...
        m_maxSpin->setValue(DEFAULT_Y_MAX);
        m_maxSpin->setSingleStep(Y_STEP);
...
void CellVoltageBarPlugin::updateState(double current_time)
{
    ...
    QVector<double> values(CELL_COUNT, 0.0);
    for (int i = 0; i < CELL_COUNT; i++) {
...
void CellVoltageBarPlugin::onMinValueChanged(double val)
{
    if (val >= m_maxSpin->value()) {
        m_minSpin->blockSignals(true);
        m_minSpin->setValue(m_maxSpin->value() - Y_STEP);
        m_minSpin->blockSignals(false);
        val = m_minSpin->value();
    }
    if (m_barWidget) {
...
void CellVoltageBarPlugin::onMaxValueChanged(double val)
{
    if (val <= m_minSpin->value()) {
        m_maxSpin->blockSignals(true);
        m_maxSpin->setValue(m_minSpin->value() + Y_STEP);
        m_maxSpin->blockSignals(false);
        val = m_maxSpin->value();
    }
    if (m_barWidget) {
...
void CellVoltageBarPlugin::detectCellChannels()
{
    ...
    QString info = QString("Found %1/%2 channels.").arg(foundCount).arg(CELL_COUNT);
    if (foundCount < CELL_COUNT) {
